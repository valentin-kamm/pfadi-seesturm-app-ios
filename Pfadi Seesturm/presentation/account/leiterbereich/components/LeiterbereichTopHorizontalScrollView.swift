...
struct LeiterbereichTopHorizontalScrollView<F: NavigationDestination>: View {
    ...
    private let foodState: UiState<[FoodOrder]>
    private let foodNavigationDestination: F
    
    init(
        foodState: UiState<[FoodOrder]>,
        foodNavigationDestination: F
    ) {
        self.foodState = foodState
        self.foodNavigationDestination = foodNavigationDestination
    }
...
#Preview("Loading") {
    LeiterbereichTopHorizontalScrollView(
        foodState: .loading(subState: .loading),
        foodNavigationDestination: AccountNavigationDestination.food(user: DummyData.user1)
    )
}
#Preview("Error") {
    LeiterbereichTopHorizontalScrollView(
        foodState: .error(message: "Schwerer Fehler"),
        foodNavigationDestination: AccountNavigationDestination.food(user: DummyData.user1)
    )
}
#Preview("Success") {
    LeiterbereichTopHorizontalScrollView(
        foodState: .success(data: DummyData.foodOrders),
        foodNavigationDestination: AccountNavigationDestination.food(user: DummyData.user1)
    )
}
