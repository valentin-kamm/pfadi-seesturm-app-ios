...
class AnlaesseService: WordpressService {
    ...
    let repository: AnlaesseRepository
    init(repository: AnlaesseRepository) {
        self.repository = repository
    }
    
    func fetchEvents(calendar: SeesturmCalendar, includePast: Bool, maxResults: Int) async -> SeesturmResult<GoogleCalendarEvents, NetworkError> {
    ...
    func getNext3Events(calendar: SeesturmCalendar) async -> SeesturmResult<GoogleCalendarEvents, NetworkError> {
        await fetchFromWordpress(
            fetchAction: { try await self.repository.getNext3Events(calendar: calendar) },
            transform: { try $0.toGoogleCalendarEvents() }
        )
    }
}
